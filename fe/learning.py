...
<html>
  ...
  <body>
    ...
    <script>
    function sendpost(base_url, data, i) {
        $.post(base_url, data, function (rp) {
            $('.response').html('<font color="green"><b>请求结果: 请求完成' + i + '次！' + '</b></font>' + rp
            );
        });
    }
    </script>
...
def main(HOST, PORT):
    # certfile = write_cert()
    # httpd = HTTPServer((HOST, PORT), LearningHTTPRequestHandler)
    # httpd.socket = ssl.wrap_socket(httpd.socket, certfile=certfile, server_side=True)
    server_address = (HOST, PORT)
    httpd = HTTPServer(server_address, LearningHTTPRequestHandler)
    httpd.serve_forever()
